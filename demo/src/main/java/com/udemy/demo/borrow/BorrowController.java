...
public class BorrowController {
    ...
    public ResponseEntity list(Principal principal) {
        List<Borrow> borrows = borrowRepository.findByBorrowerId(BookController.getConnectedUserId(principal));
        return new ResponseEntity(borrows, HttpStatus.OK);
    }

    @PostMapping("/borrows/{bookId}")
    public ResponseEntity create(@PathVariable("bookId") String bookId, Principal principal) {
        Integer userConnectedId = BookController.getConnectedUserId(principal);
